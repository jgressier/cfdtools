...
class WriterRestartIC3():
    ...
    def __init__(self, filename, endian='native'):
        ...
        '''
        print
        api.io.print('std',":: WRITER RESTART IC3 ::")
        print
    ...
    def set_mesh(self, xyz, co):
        ...
        #                                                                 reverse)
        # if hasNegVols:
        #     api.io.print('std',"\tWarning. Negative volumes detected. Inverting all the connectivities.")
        #     reverse = int(1)
        #     trico, quaco, fa2cv, vtx2fa, ntri, nqua, hasNegVols = getfaceco(self.coordinates,
        #                                                                     self.e2v["hex"], self.e2v["pri"], self.e2v["pyr"], self.e2v["tet"],
        #                                                                     self.params["no_count"], self.params["cv_count"],
    ...
    def set_bocos(self, bocos, nboco=None):
        ...
        if nboco is None:
            ...
            if nboco != c_nboco:
                if gotInt and c_nboco == nboco+1:
                    ...
                    if gotInt:
                        c_nboco -= 1
                    api.io.print('std',"Fatal error. Incoherence between the bocos dictionary (%d bocos) and the number (%d) of bocos declared. Exiting."%(c_nboco, nboco))
                    exit()
        ...
        for key, boco in bocos.items():
            ...
            if not "slicing" in boco.keys():
                api.io.print('std',"Fatal error. No slicing found for boundary condition %s. Exiting."%(key))
                exit()
            else:
                if "int" in key.lower() or "interior" in key.lower() or "fluid" in key.lower():
                    passed_key = key
                else:
                    api.io.print('std',"\tHandling boundary condition %s.."%key)
            ...
            if "periodic" in key.lower() or \
               "periodic" in self.bocos[key]["kind"]:
                if "periodic_transform" in boco.keys():
                    ...
                    api.io.print('std',"Fatal error. Periodic (a priori) boco (%s) found but no information on the transform can be found. Exiting."%(key))
                    exit()
        ...
        if passed_key is not None:
            api.io.print('std',"\tHandling boundary condition %s.."%passed_key)
    ...
    def __WriteRestartConnectivity(self):
        ...
        header.write(self.fid, self.endian)
        # Node count
        BinaryWrite(self.fid, self.endian, "i"*self.params["fa_count"], self.f2v["noofa_i"].tolist())
        ...
        BinaryWrite(self.fid, self.endian, "i"*self.params["fa_count"]*2, self.f2e.tolist())
...
def BinaryWrite(bfile, form, varargs):
    ...
    try:
        ...
        api.io.print('std',"Fatal error. Could not write to %s. Exiting."%(bfile.name))
        exit()
