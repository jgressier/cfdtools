...
class reader(binreader):
    ...
    def _ReadRestartVar(self):
        ...
        api.io.print('std', " " * 4 + " First the scalars...")
        reset_offset = True
        while True:
            h = restartSectionHeader()
            if not h.readVar(
            ...
            if h.idata[0] == no_count:
                ...
                self.variables["cells"][h.name].set_data(var)
            else:
            ...
            api.io.print(
                ...
                % (h.name, ' ' * (20 - len(h.name)), var.min(), np.mean(var), var.max()),
            )
        api.io.print('std', " " * 4 + " end of scalars")

        # Then the vectors
        api.io.print('std', " " * 4 + " Then the vectors...")
        reset_offset = True
        while True:
            h = restartSectionHeader()
            if not h.readVar(
            ...
            if h.idata[0] == no_count:
                ...
                self.variables["cells"][h.name] = _data.celldata(ndof=ndof)
                self.variables["cells"][h.name].set_data(vector)
        ...
        api.io.print('std', " " * 4 + "  end of vectors")

        # Then the tensors
        api.io.print('std', " " * 4 + " Then the tensors...")
        reset_offset = True
        while True:
            h = restartSectionHeader()
            ...
            if h.idata[0] == cv_count:
                ...
                api.io.print('std', "Fatal error. Incoherence in dataset %s. Exiting." % (h.name))
                exit()
        api.io.print('std', " " * 4 + " end of tensors")
